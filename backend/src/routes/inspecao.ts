...
router.post('/upload-foto', async (req, res) => {
  try {
    ...
    try {
      ...
      if (error) {
        console.error('[inspecao] Erro ao salvar no Supabase:', error);
...
router.get('/portal/equipamento/:equipamentoId', async (req, res) => {
  try {
    ...
    console.log(`[inspecao.ts] ${enrichedData.length} inspeções encontradas para o equipamento`);
